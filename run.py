...
if not os.path.exists(output_dir):
    st.error(f"未找到目录 '{output_dir}'，请确保该目录存在。")
else:
    # 自动读取 output 目录下的所有 xlsx 文件
    file_list = [f for f in os.listdir(output_dir) if f.endswith('.xlsx')]
    ...
    if file_list:
        ...
        # 读取文件
        df = pd.read_excel(os.path.join(output_dir, selected_file), engine='openpyxl')
        ...
        st.error(f"在 '{output_dir}' 目录中未找到任何 xlsx 文件，请检查该目录是否有有效的文件。")
